...
@onready var player_sprite : AnimationPlayer = $PlayerBody/AnimationPlayer
...
func _ready() -> void:
	...
	GameManager.fire_attack.connect(handle_shooting)
...
func _on_collision_body_entered(body):
	if body.is_in_group("Traps") and !GameManager.god:
		hit_trap.emit()
	if !can_damage : return
	if body.is_in_group("Enemy") and !GameManager.god:
...
func shoot():
	...
	var dir = Vector2(cos(angle) * sign_x, -sin(angle))
	get_parent().add_child(bullet)
	bullet.shoot(dir, 2000, bullet_lifetime)
	shoot_cooldown_timer = shoot_cooldown_time
...
func use_potion(type: String) -> void:
	if type == "speed":
		move_speed = 1000
